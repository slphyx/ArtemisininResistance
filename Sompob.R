    ...
    gamma,ec50,emin,emax,t,runmax)
{
  ...
  mod[c(0,4,8,12,18,24,30,36,42,48,54)+1,]
}


## testing the model with sliders
## this requires 'manipulate' package
library("manipulate")

##outls <- distribution of parasites during treatment
outls<-ss.model(1000000,10,5,6,48,c(6,24,25,38,39,44),"dhaConc.txt",24,7,
                c(5.6,5.6,5.6),c(20.5,20.5,20.5),c(0,0,0),c(99.99,99.999,99.99),6.5,192)


plotX<-function(ls){
  plot(ls,type="l",xlim=c(0,100),ylim=c(7,13),xlab="time(hrs)",ylab=expression("log10(parasitaemia)/"*mu*"L"))
  abline(h=8,lty=2)
}
...
manipulate(
  plotX(ss.model(10^11,10,20,6,48,c(6,26,27,38,39,44),"dhaConc.txt",dinv,ndrg,
                 c(6.5,6.5,6.5),c(20.5,20.5,20.5),c(0,0,0),c(emR,emT,emS),T4alpha,192)),
...
#  e50R=slider(0.001,100,20.5,label="Ring EC50"),
#  e50T=slider(0.001,100,20.5,label="Trop EC50"),
#  e50S=slider(0.001,100,20.5,label="Schi EC50"),
